...
void MACRO_Integration_Q2Nu()
{
  TFile* fsource = new TFile("corr_data_Pt2_processed.root","READ");
  TFile* ftarget = new TFile(Form("meanPt2_Zh_%i.root",N_Zh),"RECREATE");
...
void integration(TFile* ftarget, TFile* fsource, TString material)
{
  ...
  TH1F* h_Zh = new TH1F("corr_data_"+material+"_Zh","",N_Zh,Zh_limits);
  
  for(Int_t Zh_bin = 0 ; Zh_bin < N_Zh ; Zh_bin++)
    {	
      TH1F* h_Pt2_INT       = new TH1F(Form("corr_data_"+material+"_Pt2_%i",Zh_bin),"",N_Pt2,Pt2_min,Pt2_max);
      for(Int_t Nu_bin = 0 ; Nu_bin < N_Nu ; Nu_bin++)
	{
      ...
      h_Zh->SetBinContent(Zh_bin+1, h_Pt2_INT->GetMean());
      h_Zh->SetBinError(Zh_bin+1, h_Pt2_INT->GetMeanError());
      
      delete h_Pt2_INT;
    }// End Zh loop
  
  ftarget->cd();
  h_Zh->Write("meanPt2_"+material+"_CLEAN_INTERPOLATED");
